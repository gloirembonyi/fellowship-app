    ...
    if (updatedApplication.email && updatedApplication.firstName) {
      try {
        ...
        if (status === 'approved') {
          ...
          const documentSubmissionUrl = `${process.env.NEXT_PUBLIC_APP_URL || 'http://197.243.28.38:4000'}/documents/${id}`;
    ...
    if (application.email && application.firstName) {
      try {
        ...
        if (status === "rejected") {
          ...
          // Generate document submission URL
          const documentSubmissionUrl = `${process.env.NEXT_PUBLIC_APP_URL || 'http://197.243.28.38:4000'}/documents/${applicationId}`;
